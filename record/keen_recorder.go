...
// A KeenRecorder is responsible for recording events into Keen.io's data
// collection service. It uses the given projectId and writeKey, meaning that a
// KeenRecorder is scoped to the project context. All methods on it are blocking, so
// it may be best to spawn go-routines for high-throughput event generators.
//
// KeenRecorder implements the Recorder interface.
type KeenRecorder struct {
...
var _ Recorder = new(KeenRecorder)

// New returns a pointer to a new instance of a KeenRecorder. It passes along the
// given projectId and writeKey, which will be used in all future requests.
func New(baseUrl *url.URL, projectId, writeKey string) *KeenRecorder {
	return &KeenRecorder{
		baseUrl:   baseUrl,
		projectId: projectId,
		writeKey:  writeKey,
...
func (r *KeenRecorder) Record(e Event) error {
...
func (r *KeenRecorder) responseToError(resp *http.Response) error {
...
func (r *KeenRecorder) request(e Event) (*http.Response, error) {
...
func (r *KeenRecorder) url(e Event) (*url.URL, error) {
	url, err := url.Parse(
